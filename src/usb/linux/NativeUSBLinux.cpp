...
#include "usb/NativeUSB.h"

/* Definitions and macros */

#define MAX_USB_DEVICES             127
#define BULK_TIMEOUT                1000000000 //!< millisec

//! Tell gcc not to warn about a particular variable being unused.  This is 
//! useful for function parameters that are required by an interface prototype, 
//! but not for the actual implementation.  This also mangles the name so that 
//! an error will be generated if anything tries to use an UNUSED variable.
...
static __device_instance_t* __lookup_device_instance_by_location(const char *bus_location, const char *device_location);
static __device_instance_t* __add_device_instance               (const char *bus_location, const char *device_location, int vendorID, int productID);
...
//! This function will iterate over the known devices and attempt to match the given ID.  
static __device_instance_t *__lookup_device_instance_by_ID(long deviceID) {
    int valid = 0;
    for (int i = 0; i < MAX_USB_DEVICES && valid < __enumerated_device_count; i++) 
    {
        if (__enumerated_devices[i].valid) 
        {
...
static __device_instance_t* __lookup_device_instance_by_location(const char *bus_location, const char *device_location) 
{
    ...
    for (int i = 0; i < MAX_USB_DEVICES && valid < __enumerated_device_count; i++) 
    {
        if (__enumerated_devices[i].valid) 
        {
            if (!strncmp(__enumerated_devices[i].bus_location,    bus_location,    (PATH_MAX + 1)) && 
                !strncmp(__enumerated_devices[i].device_location, device_location, (PATH_MAX + 1))) 
                return &(__enumerated_devices[i]);
            valid++;
        }
    }
...
static void __purge_unmarked_device_instances(int vendorID, int productID) 
{
    ...
    for (int i = 0; i < MAX_USB_DEVICES && valid < __enumerated_device_count; i++) 
    {
        __device_instance_t* device = &(__enumerated_devices[i]);
         ...
         * discarded.
         */
        if(!device->mark && (vendorID == device->vendorID) && (productID == device->productID)) 
        {
            ...
            memset(&__enumerated_devices[i], 0, sizeof(*device));
        } 
        else 
        {
...
//! Call this function to trigger usb_find_busses() and usb_find_devices()
//! (updates global usb_busses table).
//! 
...
static int  __probe_devices() 
{
    ...
    // Check whether Device count changed; this could be useful 
    // for responding to hotplug events.
