	...
	@for VS in $(VISUALSTUDIO_PROJECTS) ; \
     do \
        $(RM) -rf os-support/windows/$$VS/*.suo \
                  ...
                  os-support/windows/$$VS/VSProj/WasatchCPP.vcxproj.user \
...
doc-user: 
	...
	@( cd doc && doxygen Doxyfile > doxygen-user.out && mv rtf/refman.rtf rtf/User_Manual.rtf )

doc-dev: 
	@echo $(DOXYGEN_BASE)doxygen is generating the developer docs...
	@( cd doc && doxygen Doxyfile-dev > doxygen-dev.out && mv rtf/refman.rtf rtf/Developer_Manual.rtf )
